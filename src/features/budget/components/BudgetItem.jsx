...
export function BudgetItem({ item, onEdit, onDelete }) {
  ...
  return (
    <div className="grid grid-cols-6 gap-4 py-5 px-8 items-center hover:bg-pink-50/30 transition-colors group">
      ...
      <div className="col-span-1 text-right font-medium text-gray-500">
      ...
      <div className="col-span-1 flex items-center justify-between pl-4">
        {isPaid ? (
          <button className="px-5 py-2 bg-pink-500 text-white rounded-full text-xs font-bold shadow-sm cursor-default">Paid</button>
        ) : (
          <button className="px-5 py-2 border-2 border-pink-500 text-pink-500 hover:bg-pink-50 rounded-full text-xs font-bold transition-colors cursor-default">Unpaid</button>
        )}
